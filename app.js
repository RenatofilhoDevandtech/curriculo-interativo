  ...
  const projectsData = [
    {
      ...
      description:
        "Este próprio currículo, desenvolvido com HTML, TailwindCSS, Node.js e a API do Gemini para fornecer análises interativas e em tempo real.",
      technologies: [
        "HTML",
        "CSS",
        "TailwindCSS",
    ...
    },
    {
      ...
      description:
        "Documentação da minha imersão de mais de 400 horas no programa AWS re/Start, traduzindo conceitos complexos de nuvem em guias práticos e analogias do mundo real.",
    ...
    },
    {
      title: "Site-Primefix",
      description:
        "PrimeFlix Moderno é uma aplicação de streaming premium, pensada para entregar uma experiência fluida, envolvente e visualmente marcante. Cada detalhe — da arquitetura ao design — foi cuidadosamente planejado.",
      technologies: [
        "React",
        "JavaScript",
        "CSS",
    ...
    },
    {
      ...
      description:
        "O frontend do Elevate é a camada interativa e dinâmica da nossa plataforma, projetada para proporcionar uma experiência de usuário (UX) intuitiva, responsiva e envolvente.",
      technologies: [
        "React",
        "JavaScript",
        "HTML",
    ...
    },
    {
      ...
      description:
        "O Gestor Integrado Inteligente (GII), desenvolvido no O-HACKA-TA-ON com Microsoft Power Platform, é uma solução inovadora para eliminar dores crônicas na gestão de processos.",
      technologies: [
        "React",
        "Microsoft",
        "Power BI",
    ...
    },
    {
      ...
      description:
        "Desafio técnico focado na criação de uma página web responsiva utilizando HTML, CSS, SASS e React com React Router.",
  ...
  const UIModule = {
        ...
        .map(
          ...
          <div class="bg-slate-800/50 p-6 rounded-xl border border-white/10">
            <div class="flex justify-between items-start">
                <h4 class="font-bold text-blue-300 text-lg">${
                  project.title
                }</h4>
                ${
                  project.link !== "#"
                    ? `<a href="${project.link}" target="_blank" class="text-sm text-indigo-300 hover:underline">Ver Projeto <i class="fa-solid fa-arrow-up-right-from-square text-xs"></i></a>`
                    : ""
                }
            </div>
            ...
            <div class="flex flex-wrap gap-2 mt-4">
              ${project.technologies
                .map(
                  (tech) =>
                    `<span class="bg-indigo-500/20 text-indigo-300 text-xs font-medium px-2.5 py-0.5 rounded-full">${tech}</span>`
                )
                .join("")}
            </div>
          </div>
    ...
    setupEventListeners: () => {
      ...
      modalToggle?.addEventListener(
        "change",
        () =>
          (document.title = modalToggle.checked
            ? "Você achou um easter egg! ✨"
            : "Renato Filho | Desenvolvedor Fullstack")
      );
    },
    streamText: (element, text, onComplete) => {
      ...
      const interval = setInterval(() => {
        if (i < words.length) {
          ...
          element
            .closest("#skill-modal-body")
            ?.scrollTo(0, element.closest("#skill-modal-body").scrollHeight);
          i++;
        } else {
  ...
  const FirebaseModule = {
    ...
    init: async () => {
      ...
      try {
        ...
        document
          .getElementById("view-counter-container")
  ...
  const GeminiModule = {
    ...
    getCVContext: () => {
      ...
      const sections = {
        ...
        "Sobre Mim (História Completa)": aboutArticle
          ? aboutArticle.innerText
          : "",
        Habilidades: Array.from(document.querySelectorAll(".skill-tag"))
          .map((el) => el.firstChild.textContent)
          .join(", "),
        Experiências: Array.from(document.querySelectorAll(".experience-item"))
          .map((el) => el.innerText.replace(/\n/g, " - "))
          .join("; "),
        Educação: Array.from(document.querySelectorAll("#education-list > div"))
          .map((el) => el.innerText.replace(/\n/g, " - "))
          .join("; "),
    ...
    setupEventListeners: () => {
      ...
      const suggestionChipsContainer =
        ...
        suggestions.forEach((suggestion) => {
          ...
          chip.addEventListener("click", () => {
            const targetElement = document.querySelector(
              suggestion.targetSelector
            );
            if (targetElement) {
              targetElement.click();
            } else {
              console.warn(
                `Elemento alvo para o chip não encontrado: ${suggestion.targetSelector}`
              );
            }
          });
      ...
      const handleApiCall = async (button, action) => {
        ...
        const { systemPrompt, userQuery, userQueryText, suggestions } =
          action.getPrompt();
        ...
        try {
          ...
          const errorTarget = action.modal
            ? document.querySelector("#ai-bubble-container .ai-bubble")
            : action.displayTarget;
      ...
      document
        .getElementById("analyze-job-btn")
      ...
      document
        .getElementById("clear-analysis-btn")
        ?.addEventListener("click", () => {
          document.getElementById("job-description").value = "";
          document
            .getElementById("gemini-result-container")
            .classList.add("hidden");
        });

      // Botão "Por que me contratar?"
      document
        .getElementById("why-hire-me-btn")
        ?.addEventListener("click", (e) =>
          handleApiCall(e.currentTarget, {
            name: "Por que me contratar?",
            modal: true,
            getPrompt: () => ({
              ...
              suggestions: [
                {
                  label: "Fale sobre a experiência dele",
                  targetSelector:
                    '[data-experience-title="Analista de Suporte N1"]',
                },
                {
                  label: "Ver projeto principal",
                  targetSelector: "#projects-list a",
                }, // Seleciona o primeiro link de projeto
              ],
            }),
          })
        );
      ...
      // Tags de Habilidades
      document
        .querySelectorAll(".interactive-item.skill-tag")
        .forEach((tag) => {
          ...
          tag.addEventListener("click", () => {
            handleApiCall(tag, {
              name: "Análise de Habilidade",
              modal: true,
              getPrompt: () => ({
                userQueryText: `Me fale mais sobre a habilidade: ${skillName}`,
                systemPrompt:
                  "PERSONA: Você é o assistente de carreira do Renato. Fale em primeira pessoa ('Ele usou...', 'No currículo dele vejo que...'). SEJA ACESSÍVEL: Evite jargões. TAREFA: Gere 2-3 pontos curtos (*) sobre como o Renato aplicou uma habilidade, conectando-a aos projetos ou à história de vida dele.",
                userQuery: `Explique como o Renato aplicou "${skillName}". Contexto: ${GeminiModule.getCVContext()}`,
                suggestions: [
                  {
                    label: "Qual sua principal soft skill?",
                    targetSelector: '[data-skill-name="Resiliência"]',
                  },
                  {
                    label: "Fale sobre React",
                    targetSelector: '[data-skill-name="React"]',
                  },
                ],
              }),
            });
      ...
      document
        .querySelectorAll(".interactive-item.experience-item")
        .forEach((item) =>
            ...
            handleApiCall(item, {
              ...
              getPrompt: () => {
                ...
                return {
                  ...
                  suggestions: [
                    {
                      label: "Por que ele mudou de carreira?",
                      targetSelector:
                        '[data-experience-title="transition-to-tech"]',
                    },
                    {
                      label: "Qual a sua principal habilidade?",
                      targetSelector: '[data-skill-name="React"]',
                    },
                  ],
                };
